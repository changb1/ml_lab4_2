...
# Env settings
from ptdqn_env_old import ptdqn as ptdqnEnv
...
def load_data(year, month):
    ...
    for test_months in range(TEST_MONTHS):
        ...
        with open(PairsDataYYYYMM_Dir + str(test_year) + str(test_month).zfill(2) +'_new_without45_ln'+ '.json', 'r') as f:
    ...
    for train_months in range(TRAIN_MONTHS):
        ...
        with open(PairsDataYYYYMM_Dir + str(year) + str(month).zfill(2) +'_new_without45_ln'+ '.json', 'r') as f:
            pairs_dict_temp = json.load(f)
